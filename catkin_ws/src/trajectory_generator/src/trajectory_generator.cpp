...
#define use_only_first_path true
#define alpha_yaw 0.5
...
public:
	TrajectoryGeneratorClass(ros::NodeHandle& node)
	{
        ...
        counter_print = 0;
        yaw = 0;
        flag_received_first_plan = false;
	...
	{
		if ((use_only_first_path && !flag_received_first_plan) || (!use_only_first_path)){
			path = *msg;
			// ------ reset ---------
			distances.clear();
			adapted_times.clear();
		...
		if (use_only_first_path && !flag_received_first_plan){
			flag_received_first_plan = true;
		}
	}
	...
	void loop()
	{
        ...
        if (flag_received_new_path){
        	...
        	int acc_dec_points = std::min((int)(N_points_in_path/7),50);
            if (index_actual_segment < acc_dec_points){   //Acceleration in 1/7 points or for maximum 50 points (~2.5meters)
                ds = ds_vector[index_actual_segment]*(1.0/(acc_dec_points+1))*(index_actual_segment+1);
                /*if (counter_print >= 10){
                    ROS_INFO("TrajectoryGenerator: in acceleration: %f",ds);
                }*/
            } else if (index_actual_segment >= N_points_in_path - acc_dec_points){   //Deceleration in 1/7 points or for maximum 50 points (~2.5meters)
                ds = ds_vector[index_actual_segment]*(1.0/(acc_dec_points+1))*(N_points_in_path-index_actual_segment);
                /*if (counter_print >= 10){
