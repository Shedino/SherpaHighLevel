...
void callback(const PointCloud::ConstPtr& msg)
{
  ...
  if(min_point.x > 0 && min_point.z<=OBSTACLE_DETECTION) //obstacle on the right side
  {
	  if(min_point.z > GO_BACK_LIMIT) {
		  printf("Turn Left\n");
		  velocity_.channel[STEERING] = LEFT;
	  }
	  else {
		  printf("Turn Right\n");
		  velocity_.channel[STEERING] = RIGHT;
	  }
  }
  ...
  {
	  printf("Go Straight\n");
	  velocity_.channel[STEERING] = STRAIGHT;
  ...
  {
	  printf("Go Forward\n\n");
	  velocity_.channel[THROTTLE] = FORWARD;
  }
  else
  {
	  printf("Go Back\n\n");
	  velocity_.channel[THROTTLE] = BACK;
  }
...
int main(int argc, char** argv)
{
  ...
  velocity_publisher_ = nh1.advertise<casy_rover::RC>("send_rc", 10);

  velocity_.channel[STEERING] = STRAIGHT;
  velocity_.channel[THROTTLE] = 1470;
